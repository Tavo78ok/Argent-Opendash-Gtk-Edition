...
def hw_procs_top():
    """Top procesos por CPU con limite estable para no saturar layouts GTK."""
    def _read():
        return sorted(
            psutil.process_iter(['pid','name','cpu_percent','memory_percent']),
            key=lambda p: p.info['cpu_percent'] or 0,
            reverse=True)
    procs = safe_hw(_read, timeout=3.0, default=[])
    hdr   = f"{'PID':<8}{'PROCESO':<22}{'CPU%':<8}{'RAM%'}\n" + '─'*50 + '\n'
    # Se limita a los top 15 procesos para resguardar el hilo principal de renderizado
    rows  = ''.join(
...
def _run_in_terminal(script, need_root=True):
    ...
    for term in ['x-terminal-emulator','mate-terminal','gnome-terminal',
        ...
        try:
            # Adaptación de argumentos para terminales modernas vs heredadas
        ...
        def _r(d,r):
            if r=='ok':
                def _l():
                    ...
                    GLib.idle_add(lambda: run_bg(self._load_apt))
                run_bg(_l)
        ...
        def _r(d,r):
            if r=='ok':
                def _l():
                    ...
                    GLib.idle_add(lambda: run_bg(self._load_flat))
                run_bg(_l)
        dlg.connect('response',_r); dlg.present()
    ...
    def _svc_action(self, name, action):
        def _run():
            try:
                ...
                GLib.idle_add(lambda: run_bg(self._load_svcs))
            except Exception as e:
